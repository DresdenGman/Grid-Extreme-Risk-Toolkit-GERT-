  ...
  return (
    <div className="flex h-full flex-col justify-between gap-7">
      <div className="flex items-start justify-between gap-4">
        <div>
          <span className="technical-label text-[#87847e]">Forecast target</span>
          <span className="mt-1 block text-base font-medium text-[#141414]">{targetLabel}</span>
        </div>
        ...
        {QUANTILES.map(({ key, label, note, color }, index) => {
          ...
          return (
            <div key={key} className="grid grid-cols-[48px_1fr_76px] items-center gap-3">
              <div><span className="block text-xs font-semibold text-[#141414]">{label}</span><span className="technical-label text-[8px] text-[#87847e]">{note}</span></div>
              <div className="relative h-7 overflow-hidden rounded-sm bg-black/[0.035]">
                <div
                  className="h-full rounded-sm transition-[width] duration-700"
                  style={{ width: `${Math.min(100, (value / maximum) * 100)}%`, background: color, opacity: index === 3 ? 1 : 0.72 }}
                />
              </div>
              <span className="text-right font-mono text-xs text-[#141414]">
